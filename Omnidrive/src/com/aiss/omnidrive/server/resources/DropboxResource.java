...
public class DropboxResource {
	...
	private static final String BASE_URL_V1 = "https://api.dropboxapi.com/1";
	...
	public static DropboxUserInfo getUserInfo(String token){
		...
		connection = new ClientResource(BASE_URL_V1 + "/account/info");
	...
	public static DropboxFile getFile(String token, String path) {
		DropboxFile file;
		ClientResource connection;
		ChallengeResponse authorization;
		String jsonData;
		...
		jsonData = "{\"path\":\"" + path + "\", \"include_media_info\":false, \"include_deleted\":false, \"include_has_explicit_shared_members\":false }";
		data = new JsonRepresentation(jsonData);
		data.setMediaType(MediaType.APPLICATION_JSON);
		connection = new ClientResource(BASE_URL + "/files/get_metadata");
		connection.setMethod(Method.POST);
		authorization = new ChallengeResponse(new ChallengeScheme("OAuth2",
				TYPE_TOKEN));
		authorization.setRawValue(token);
		...
		try {		
			file = connection.post(data, DropboxFile.class);
		} catch (ResourceException e) {
			System.out.println(e.getStatus().getCode() + "");
			System.out.println(e.getStatus().getDescription() + "");
			System.out.println(e.getStatus().getReasonPhrase() + "");
