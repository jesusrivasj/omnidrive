...
public class DropboxFileView extends Composite {
	...
	public DropboxFileView(Map<String, String> params, final DropboxFile file){
		...
		if (file.isDirectory()) {
			...
			fileView.addDoubleClickHandler(new DoubleClickHandler() {
				...
				public void onDoubleClick(DoubleClickEvent event) {
					...
					fileParams.put("parent", file.getPathDisplay());
					MainController.go("dropbox", fileParams);
				}
			});
		} else {
		...
		fileView.addHandler(new ContextMenuHandler() {
			...
			public void onContextMenu(ContextMenuEvent event) {
				...
				new DropboxFileContextualMenuView(file, event);
			}
